...
def run_simulation(config: SimulationConfig) -> SimulationResult:
    ...
    def journey(p):
        ...
        p.first_doctor_queue_entry=env.now; held=yield from seat(p,'first_seat_available'); log.add(env.now,p,'first_doctor_queue_joined','doctors',len(doctors.queue))
        yield from use(p,doctors,'initial_consultation','initial_consultation_start','initial_consultation_end','initial_consultation','doctors',1); yield from release(p,held)
        ...
        p.return_consultation_queue_entry=env.now; held=yield from seat(p,'return_seat_available'); log.add(env.now,p,'return_doctor_queue_joined','doctors',len(doctors.queue))
        priority=0 if config.queue_policy.consultation_policy=='return_priority' else 1
        yield from use(p,doctors,'return_consultation','return_consultation_start','return_consultation_end','return_consultation','doctors',priority); yield from release(p,held); p.discharge_time=env.now; evaluate(p,config.satisfaction_rules,env.now); log.add(env.now,p,'patient_discharged','clinic')
